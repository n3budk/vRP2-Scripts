...
local cfg = module("vrp_dmvschool", "cfg/dmv")
local lang = module("vrp_dmvschool", "cfg/lang/"..cfg.lang)
...
function vRPdmv:setcLicense(license)
	TestDone = license
	tlock = not license
	plock = not license
end
...
function SpawnTestCar()
	...
	local vehicle = GetHashKey('fiesta')
	...
	SetEntityHeading(spawned_car, 230.84)
	...
	SetEntityAsMissionEntity(GetCar(),true,true)
end

function vRPdmv:DIntro()
	Citizen.Wait(1)
	local myPed = GetPlayerPed(-1)
	DTutOpen = true
		  ...
		  if onTestEvent == step then
		    if onTestEvent ~= #cfg.practical.steps then
			  ...
			  if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), event.pos[1], event.pos[2], event.pos[3], true) > 4.0001 then
			  	DrawMarker(1,event.pos[1], event.pos[2], event.pos[3]-1,0, 0, 0, 0, 0, 0, 1.5, 1.5, 1.5, 178, 236, 93, 155, 0, 0, 2, 0, 0, 0, 0)
			  else
			  	...
			  	onTestBlipp = AddBlipForCoord(nstep.pos[1],nstep.pos[2],nstep.pos[3]-1)
			  	N_0x80ead8e2e1d5d52e(onTestBlipp)
			  	SetBlipRoute(onTestBlipp, 1)
			  	if event.stop then
...
vRPdmv.tunnel.startIntro = vRPdmv.startIntro
vRPdmv.tunnel.payTheory = vRPdmv.payTheory
vRPdmv.tunnel.payPractical = vRPdmv.payPractical

vRPdmv.tunnel.startPractical = vRPdmv.startPractical
vRPdmv.tunnel.startTheory = vRPdmv.startTheory
vRPdmv.tunnel.setcLicense = vRPdmv.setcLicense
